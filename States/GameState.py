...
class GameState(State):
    ...
    def calculate_gold_reward(self, playerInfo, stage=0):
        ...
        if stage == 0:
            if blind == "SMALL":
                ...
                base = 10
            return base + self.calculate_gold_reward(playerInfo, 1)

        if score <= target or target <= 0:
            return 0
        ...
        bonus_levels = (overkill / target) * 5

        if bonus_levels >= stage:
    ...
    def discardCards(self, removeFromHand: bool):
        if len(self.cardsSelectedList) == 0:
            self.refillHandRecursive()
            self.cardsSelectedList.clear()
            self.playedHandName = ""
            self.playedHandNameList.clear()
        ...
        if removeFromHand and card_to_remove in self.hand:
